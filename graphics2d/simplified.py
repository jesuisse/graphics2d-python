...
def draw_line(start, end, color: _pygame.Color, width=1):
    """
    Draws a line from start (x1, y1) to end (x2, y2)
    """
    _draw.draw_line(_framework.screen, start, end, color, width)    

def draw_polyline(points, color: _pygame.Color, is_closed=False, width=1):
    """
...
def draw_filled_polygon(points, color : _pygame.Color):
    """
    Draws a polygon from a sequence of points.    
    """
    _draw.draw_filled_polygon(_framework.screen, points, color)

def draw_rect(topleft, size, color: _pygame.Color, width=1):
    """
    Draws a rectangle 
    """
    _draw.draw_rect(_framework.screen, _pygame.Rect(topleft, size), color, width)

def draw_filled_rect(topleft, size, color: _pygame.Color):
    """
    Draws a filled rectangle 
    """
    _draw.draw_filled_rect(_framework.screen, _pygame.Rect(topleft, size), color)

def draw_circle(center, radius, color: _pygame.Color, width=1):
    """
    Draws a circle at center position (x, y) with the given radius.
    """
    _draw.draw_circle(_framework.screen, center, radius, color, width)

def draw_filled_circle(center, radius, color: _pygame.Color):
    """
    Draws a filled circle at position center (which is a (x, y) tuple) 
    with the given radius and color.
    """
    _draw.draw_filled_circle(_framework.screen, center, radius, color)


def draw_text(fontname : str, fontsize : float, text : str, position, color: _pygame.Color, antialiased=True, background=None):
    """
    Draws text with a given font, size and color at a given position,
    which is a (x, y) tuple.
    """
